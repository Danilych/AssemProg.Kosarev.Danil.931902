...
void DrawLines(HWND hwnd, int windowX, int windowY)
{
    ...
    for (int i = 1; i < NUM; i++)
    {
        g_pRendertarget->DrawLine(
            D2D1::Point2(apt[i-1].x, apt[i-1].y),
            D2D1::Point2(apt[i].x, apt[i].y),
            g_pRedBrush
        );
    }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        for (i = 0; i < NUM; i++)
        {
            ...
            {
            case 1:
                apt[i].y = ((int)(cyClient / 2 * (1 - sin(TWOPI * (i - NUM / 2)  / NUM *curScale)/curScale)));
                break;
            case 2:
                apt[i].y = (int)(cyClient / 2 * (1 - cos(TWOPI * (i - NUM / 2) / NUM * curScale)/curScale));
                break;
            case 3:
                apt[i].y = (int)(cyClient / 2 * (1 - tan(TWOPI * (i - NUM / 2) / NUM * curScale)/curScale));
                break;
            case 4:
                apt[i].y = (int)(cyClient / 2 * (1 - atan(TWOPI * (i-NUM/2) / NUM * curScale) / curScale));
                break;
            case 5:
                apt[i].y = (int)(cyClient / 2 * (1 - acos(TWOPI * (i - NUM / 2) / NUM * curScale) / curScale));
                break;
            case 6:
                apt[i].y = (int)(cyClient / 2 * (1 - asin(TWOPI * (i - NUM / 2) / NUM * curScale) / curScale));
                break;
            }
        }
...
void AddMenus(HWND hwnd)
{
    ...
    AppendMenu(hScaleMenu, MF_STRING, 10, L"1x");
    AppendMenu(hScaleMenu, MF_STRING, 11, L"2x");
    AppendMenu(hScaleMenu, MF_STRING, 12, L"3x");
    AppendMenu(hScaleMenu, MF_STRING, 13, L"4x");
    AppendMenu(hScaleMenu, MF_STRING, 14, L"5x");
    AppendMenu(hScaleMenu, MF_STRING, 15, L"6x");
    AppendMenu(hScaleMenu, MF_STRING, 16, L"7x");
